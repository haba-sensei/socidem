...
header("Content-Disposition: attachment; filename=med_gratuito.xls");

$consult = ejecutarSQL::consultar("SELECT `medicos`.`nombre_completo`, `medicos`.`correo`, `medicos`.`estado`, `medicos`.`periodo_membresia`, `medicos`.`membresia`, `perfil`.`telefono`, `perfil`.`codigo_referido`, `medico_bank`.`cci` FROM `medicos`, `perfil`, `medico_bank` WHERE `medicos`.`correo` = `perfil`.`correo` AND `medicos`.`membresia` = 'Gratuito' AND `perfil`.`codigo_referido` = `medico_bank`.`token_medico` ");
$count = 0;
...
<table border="1">
    <caption> <strong > LISTA DE MEDICOS CON MEMBRESIA GRATUITA </strong> </caption>
        <tr>
            <th> ID </th>
            <th> NOMBRE MEDICO </th> 
            <th> CORREO </th>   
