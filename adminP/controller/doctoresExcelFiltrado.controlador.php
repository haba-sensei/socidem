...
while( $datos_consult =mysqli_fetch_assoc($consult)){ 
    ...
    while( $datos_consult_agenda =mysqli_fetch_assoc($consult_agenda)){  
        ...
        $fecha_lunes_pasado = strtotime(date('Y-m-d', strtotime($fecha_lunes)));

        $fecha_domingo_siguiente = strtotime(date('Y-m-d', strtotime($fecha_domingo)));
...
';
   ...
   while( $datos_consult_agenda_detalle =mysqli_fetch_assoc($consult_agenda_detalle)){  
    ...
    $fecha_lunes_pasado_detalle = strtotime(date('Y-m-d', strtotime($fecha_lunes)));

    $fecha_domingo_siguiente_detalle = strtotime(date('Y-m-d', strtotime($fecha_domingo)));
    ...
    ';
    $cons_ref_detalle = ejecutarSQL::consultar("SELECT `pagos_membresias`.`estado`, `pagos_membresias`.`token_referido`, `pagos_membresias`.`monto_reducido_token`, `pagos_membresias`.`token_referido`, `codigos_promo`.`porcentaje`, `codigos_promo`.`tipo`, `pagos_membresias`.`fecha`, `pagos_membresias`.`token_medico`, `perfil`.`correo`, `medicos`.`nombre_completo`
   ...
   while( $referidos_detalle =mysqli_fetch_assoc($cons_ref_detalle)){  
        ...
        $tipo = $referidos_detalle['tipo']; 
        if( $tipo == "porcentaje" || $tipo == "medico"){
            $tipo_format = "Interno";
        }else {
            $tipo_format = "Externo";
        }
