...
unset($_SESSION["asistente"]);
 ...
 $verAfil = ejecutarSQL::consultar("SELECT `medicos`.*, `medicos`.`correo`, `medicos`.`token_confirm` FROM `medicos` WHERE `medicos`.`token_confirm` = '$token_confirm' AND  `medicos`.`mail_confirm` = 0  ");
